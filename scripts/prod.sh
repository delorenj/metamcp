...
# Production script for MetaMCP - starts both frontend and backend in production mode
...
echo -e "${BLUE}=== MetaMCP Production Setup ===${NC}\n"
...
echo -e "\n${YELLOW}Starting frontend in production mode on port 12008...${NC}"
cd apps/frontend

# Use standalone production build
PORT=12008 pnpm start:standalone &
FRONTEND_PID=$!
cd ../..
